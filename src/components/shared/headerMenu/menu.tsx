'use client'
import { useEffect,useState } from 'react';
import Style from './menu.module.scss';
import { Button } from '@/components/UI/button/button';
import { Header_input } from '@/components/UI/input/input';
export const Menu = ({ status ,click}: { status: boolean,click:boolean }) => {


    
if(status == true) {
    return (
        <div className={`${Style.fadeIn} py-[20px] px-[30px] fixed left-0 top-[144px] translate-x-full w-full   bg-white flex justify-center gap-5 flex-col-reverse `}>
              <div className=" "><Header_input/></div>
                        <div className="flex gap-[16px] justify-center ">
                            <div><Button variant='user' status={false} size="default"  /></div>
                            <div><Button variant="cart" size="default" status={false}/></div>
                        </div>
        </div>
    );
}
else {
    if(click == true) {
return (
        <div className={`${Style.fadeOut} py-[20px] px-[30px] fixed left-0   top-[144px]  w-full  bg-white flex justify-center gap-5 flex-col-reverse `}>
              <div className="flex-[2] "><Header_input/></div>
                        <div className="flex gap-[16px] justify-center flex-1 ">
                            <div><Button variant='user' status={false} size="default"  /></div>
                            <div><Button variant="cart" size="default" status={false}/></div>
                        </div>
        </div>
    );
    }
    else {
        return (
        <div className={` py-[20px] px-[30px] fixed left-0 -translate-x-full  top-[144px]  w-full  bg-white flex justify-center gap-5 flex-col-reverse `}>
              <div className="flex-[2] "><Header_input/></div>
                        <div className="flex gap-[16px] justify-center flex-1 ">
                            <div><Button variant='user' status={false} size="default"  /></div>
                            <div><Button variant="cart" size="default" status={false}/></div>
                        </div>
        </div>
    );
    }
    
}

   
};
