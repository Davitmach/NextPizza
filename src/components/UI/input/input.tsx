...
const Ref = inputProps.ref;
 return <div className="Input_box">{Label && <div className="text-black-label font-[700] text-[14px] mb-[5px]">{Label}</div>}<input ref={Ref} type={Input_type} className={`w-full outline-none border border-white-border rounded-[10px] px-[18px] py-[13px] ${Error_state == true && Style.Error_input}`}/>
 {Error_state == true && <span className="Error_message text-red-error_message font-[400] text-[14px]">{Error_message && Error_message}</span>}
 </div>
}
export const Header_input = ()=> {
    ...
    return <div  className="Input_box relative"> <div className={`fixed left-0 top-0 w-full h-svh z-10  ${active ? Style.Active : Style.Disable}`}></div><div className={`${active ? Style.Input_active : ''} px-5 relative rounded-custom h-[50px] bg-[#F9F9F9] overflow-hidden`}><svg onClick={async()=> {
  ...
  if(Response && Array.isArray(Response) && Response.length>0) {
    ...
    }} className="cursor-pointer absolute top-[35%] left-3" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <input  ref={ref} placeholder="Поиск пиццы..." onInput={(event)=> {
        HandleInput(event)

    }} className=" bg-[#F9F9F9] px-3 w-full h-full outline-none placeholder:text-gray-light4 placeholder:text-base font-normal"/></div>{searchedValue && <div className="w-full  mt-[10px] rounded-[10px] bg-white absolute z-50"> {searchedValue.map((item,index) => (
        <div onClick={()=> {
          push(`product/${item.id}`)
          setActive(false)
          setSearchedValue([])  
        }} key={item.id} className={`cursor-pointer py-[10px] flex gap-3 mx-[19px] mt-[13px] duration-500 hover:bg-white-hover ${index == searchedValue.length-1 ? 'mb-[13px]':'mb-0'}`}>
            <div><Image alt="Pizza Image" width={30} height={30}  src={item.imageUrl}/></div>
            <div><HighlightedSearch searchValue={searchValue} itemName={item.name}/></div>
